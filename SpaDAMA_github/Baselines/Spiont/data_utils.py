...
def filter_model_genes(
    ...
    # assert st_ad.var_names.is_unique, "st_ad var_names are not unique!"
    # assert sc_ad.var_names.is_unique, "sc_ad var_names are not unique!"
    # 去除 st_ad 中重复的基因名
    st_ad = st_ad[:, ~st_ad.var_names.duplicated()].copy()

    # 或者，如果是 sc_ad 中的重复值
    sc_ad = sc_ad[:, ~sc_ad.var_names.duplicated()].copy()

    overlaped_genes = np.intersect1d(sc_ad.var_names,st_ad.var_names)
    overlaped_genes = pd.unique(overlaped_genes)
