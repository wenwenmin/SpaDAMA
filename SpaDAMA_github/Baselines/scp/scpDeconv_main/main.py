...
def main(a,b,cell_key):
	...
	for i in range(a,b):
		...
		if hasattr(st_adata.X, 'toarray'):
			st_data = pd.DataFrame(data=st_adata.X.toarray(), columns=st_adata.var_names, index=st_adata.obs_names)
		else:
		...
		"""保存途径"""
		outfile = 'Baselines/scpDeconv\Result' + '/dataset_sim' + str(i) + '/'
	...
	print(f"scp Total time taken: {total_time:.2f} seconds")  # 打印总时间