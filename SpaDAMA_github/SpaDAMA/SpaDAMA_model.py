...
class SpaDAMA(nn.Module):
    ...
    def double_train(self, sm_data, sm_label, st_data,mask_retio):
        self.train()
        self.prepare_dataloader(sm_data, sm_label, st_data, self.batch_size)
        self.optim_stage1 = torch.optim.Adam([{'params': self.encoder_da.parameters()},
                                              {'params': self.decoder_da.parameters()}],lr=self.learning_rate)
        self.optim_stage2 = torch.optim.Adam([{'params': self.Discriminator.parameters()},
                                              {'params': self.Classifier.parameters()}], lr=self.learning_rate)
        self.optim_stage3 = torch.optim.Adam([{'params': self.encoder_da.parameters()},
        ...
        for epoch in range(self.num_epochs_new):
            ...
            for i in range(1):
                for batch_idx, (source_x, source_y) in enumerate(self.train_source_loader):
                    ...
                    use_x, mask = self.mask_features(source_x.cuda(), mask_retio)
                    ...
                    if epoch==0:
                        ...
                        clas_loss = nn.BCELoss()(clas_out, source_label)
                        dis_loss = nn.BCELoss()(disc_out, source_label1)
                        clas_loss_y = nn.BCELoss()(clas_out_y, target_label_y)
            ...
            for i in range(1):
                for batch_idx, (source_x, source_y) in enumerate(self.train_source_loader):
                    ...
                    loss =(dis_loss + dis_loss_y +clas_loss +clas_loss_y)
                    all_loss_epoch += loss.data.item()
            ...
            for i in range(1):
                for batch_idx, (source_x, source_y) in enumerate(self.train_source_loader):
                    ...
                    loss1 = pred_loss
                    self.optim_stage3.zero_grad()
                    loss1.backward()
                    self.optim_stage3.step()
                    torch.cuda.empty_cache()
